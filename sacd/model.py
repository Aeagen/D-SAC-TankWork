...
class CateoricalPolicy(BaseNetwork):
    ...
    def sample(self, states):
        ...
        # z = (action_probs == 0.0).float() * 1e-8: 这行代码创建了一个名为 z 的张量。
        # 它首先检查 action_probs 中是否有概率值为 0 的元素，如果有，则将其对应位置的值设为 1，否则为 0。
        # 然后，通过 .float() 方法将布尔型张量转换为浮点型张量，并乘以 1e-8，得到一个很小的正数，
        # 用于处理 action_probs 中概率值为 0 的情况。
