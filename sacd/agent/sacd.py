...
from sacd.agent.base import BaseAgent
...
class SacdAgent(BaseAgent):
    ...
    def __init__(self, env, test_env, log_dir, max_episodes=50, num_steps=100000, batch_size=64,
                 ...
                 cuda=True, seed=0):
        super().__init__(
            env, test_env, log_dir, max_episodes,num_steps, batch_size, memory_size, gamma,
        ...
        self.policy = CateoricalPolicy(
            self.env.state_channels, self.env.num_action
            ).to(self.device)
        self.online_critic = TwinnedQNetwork(
            self.env.state_channels, self.env.num_action,
            dueling_net=dueling_net).to(device=self.device)
        # the network be used to eval
        # eval()is set to evaluation mode, no need to update weights
        ...
        self.target_entropy = \
            -np.log(1.0 / self.env.num_action) * target_entropy_ratio
    ...
    def explore(self, state):
        # Act with randomness.
        # state[None, ...]：将输入的图像数据 state 添加一个额外的维度，以匹配策略网络的输入维度。
        # torch.ByteTensor()：将添加了额外维度的图像数据转换为 torch.ByteTensor类型，
        #                    即无符号8位整数类型的张量。
        # .float() / 255.：将图像数据类型转换为 torch.float32（即浮点型），
        #                 并进行归一化处理，将像素值范围从 0-255 缩放到 0-1 之间。
        state = torch.ByteTensor(
            state[None, ...]).to(self.device).float() / 255.
        # with torch.no_grad():：创建一个上下文环境，该环境下的计算不会进行梯度计算，
        #                       用于推断过程中不需要更新梯度的情况。
        with torch.no_grad():
