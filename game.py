...
class Game():
	...
	def sample(self):
		action = np.random.randint(0, 5)  # 从 0 到 4 中随机选择一个整数作为动作
		return action
	...
	def reset(self, stage=None):
		...
		if self.render_mode == "grid":
			...
			return self.screen_buffer.transpose((2,0,1)) # "rgb"
	...
	def step(self, action:int):
		...
		if self.render_mode == "grid":
			...
			self.draw()
			pygame.pixelcopy.surface_to_array(self.screen_buffer, self.screen)
			return self.screen_buffer.transpose((2,0,1)), reward, done
